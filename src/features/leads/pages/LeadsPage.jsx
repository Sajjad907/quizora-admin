...
const LeadsPage = () => {
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      const response = await analyticsApi.exportLeads(selectedQuizId, debouncedSearch);
      
      // Create a URL for the blob and trigger download
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `quizora-leads-export-${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex flex-col h-full space-y-6 animate-reveal">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-4">
          ...
          <button
            ...
            className="btn-premium px-6 h-10 flex items-center gap-2 border border-primary/20 bg-primary text-white hover:scale-[1.02] transition-all rounded-xl font-black uppercase tracking-widest text-[9px] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
