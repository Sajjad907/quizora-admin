  ...
  const importShopifyProduct = async () => {
    ...
    try {
      ...
      const newProduct = {
        ...
        handle: data.handle,
        shopUrl: url, // Use the original URL for redirect
        reason: ''
      };
            ...
            <div className="w-full max-w-3xl space-y-8 pb-40 pt-10 px-4">
              {activeTab === 'questions' ? (
                ...
                outcomes.length === 0 ? (
                  ...
                  <div key={o.id || o._id} onClick={() => setSelectedId(o.id || o._id)} className={`group p-7 rounded-3xl animate-in fade-in slide-in-from-bottom-4 duration-500 cursor-pointer transition-all border-2 relative overflow-hidden ${selectedId === (o.id || o._id) ? 'bg-card border-primary shadow-2xl shadow-primary/10 scale-[1.02] z-10' : 'bg-card/40 backdrop-blur-sm border-border/50 hover:border-primary/40 hover:bg-card hover:translate-y-[-4px] shadow-sm'}`}>
                    ...
                    {(selectedOutcome.recommendedProducts || []).map((p, ip) => (
                      <div key={ip} className="p-4 bg-card rounded-2xl border border-border space-y-3 animate-in fade-in duration-300 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="space-y-1">
                            <span className="text-[8px] font-bold uppercase text-muted-foreground ml-1">Redirect URL</span>
                            <input type="text" placeholder="https://..." value={p.shopUrl || ''} onChange={(e) => { const newO = [...outcomes]; const oIdx = newO.findIndex(o => (o.id || o._id) === selectedId); newO[oIdx].recommendedProducts[ip].shopUrl = e.target.value; setOutcomes(newO); }} className="w-full h-9 bg-muted/50 px-3 rounded-lg text-[10px] font-bold outline-none border border-border focus:border-primary/40 transition-all shadow-inner" />
                          </div>
                        </div>
                        <div className="space-y-1">
